...
const { searchResult: filteredOptions } = useFuzzySearch<CSelectOption<T>>({
  search: searchQuery,
  data: computed(() => options.value), // 使用响应式引用中的值并包装为 computed
  options: {
    keys: ['label'],
    shouldSort: false,
    threshold: 0.3,
...
// 监听 options 的变化，更新 selectedOption
watch(
  options,
  (newOptions) => {
    // 更新 selectedOption
    const option = newOptions.find((option: CSelectOption<T>) => option.value === value.value);
    if (option) {
      selectedOption.value = option;
    }
    else {
...
function handleKeydown(event: KeyboardEvent) {
  ...
  if (isEnter) {
    ...
    if (valueCanBeSelected) {
      const optionToSelect = filteredOptions.value[focusIndex.value] as CSelectOption<T>;
        ...
        <div v-show="isOpen" class="c-select-dropdown" absolute z-10 mt-1 max-h-312px w-full overflow-y-auto pretty-scrollbar>
          ...
          <template v-else>
            <div
              ...
              @click="selectOption({ option: option as CSelectOption<T> })"
            >
              {{ option.label }}
            </div>
          </template>
