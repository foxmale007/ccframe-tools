 ...
 * @returns {number}
 */
function getShortIdLength(alphabetLength: number): number {
  return Math.ceil(Math.log2(2 ** 128) / Math.log2(alphabetLength));
}
 ...
 * @returns {string}
 */
function hexToCustomBase(hex: string, alphabet: string): string {
 ...
 * @returns {string}
 */
function shortenUUID(longId: string, alphabet: string, paddingParams?: PaddingParams): string {
...
function generateUUID(): string {
  ...
  s[14] = '4';
  s[19] = hexDigits.charAt((Number.parseInt(s[19], 16) & 0x3) | 0x8);
  s[8] = s[13] = s[18] = s[23] = '-';
 ...
 * @returns {string}
 */
function uuid(format: UUIDFormat = UUIDFormat.Standard): string {
  const standardUUID = generateUUID();
  if (format === UUIDFormat.Standard) {
    return standardUUID;
  }
  if (format === UUIDFormat.StandardCompact) { return standardUUID.split('-').join(''); }
